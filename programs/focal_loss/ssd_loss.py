...
class MultiboxLoss(object):
    def __init__(self, num_classes, alpha=1.0, neg_pos_ratio=3.0, background_label_id=0, negatives_for_hard=100.0):
        ...
        if background_label_id != 0: raise Exception('Only 0 as background label id is supported')
    ...
    def _softmax_loss(self, y_true, y_pred):
        ...
        softmax_loss = -tf.reduce_sum(0.25 * (tf.pow((1. - y_pred), 2)) * (y_true * tf.log(y_pred)), axis=-1)
        return softmax_loss
